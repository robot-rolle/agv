 ...
 * @Date: 2022-07-14 16:38:19
 * @LastEditors: robot_luoer@163.com robot_luoer@163.com
 * @LastEditTime: 2022-08-31 17:17:18
 * @FilePath: /hgfirst/src/hg_robot_agv/src/hg_robot.cpp
 */
#include "hg_robot.h"
#include "hg_robot_data.h"
sensor_msgs::Imu  Gy95t;
int main(int argc, char** argv)
{
	ros::init(argc,argv,"hg_robot"); //发布机器人话题
	launch_agv  hg_agv;//创建机器人对象
	hg_agv.Task();//任务循环
	return 0;
}
/**
 * @Date: 2022-07-24 17:28:04
...
{			
	...
	Robot_Vel.Angle += Robot_Vel.w * Robot_Vel.t; //The angular displacement about the Z axis, in rad //绕Z轴的角位移，单位：rad   Robot_Vel是角速度，即 yaw轴的速度
	Robot_Pos.Z = Robot_Vel.Angle;  //角度偏移量 存入ros话题信息包
	// Robot_Pos.Y =Esp_Origin_Data.data10.xC;
	// Robot_Pos.X =Esp_Origin_Data.data10.yC;
	// Robot_Pos.Z =Esp_Origin_Data.data10.radianSpeed;
	// Robot_Vel.Angle =Esp_Origin_Data.data10.radian;
//	 ROS_INFO("x:[%f],y:[%f],xc:[%f],yc:[%f]",Robot_Pos.X,Robot_Pos.Y,Esp_Origin_Data.data1.xC,Esp_Origin_Data.data1.yC);
	//  ROS_INFO("O:w:[%f],angle:[%f],x:[%f],y:[%f],[%f]",Robot_Vel.w,Robot_Pos.Z,Robot_Pos.X,Robot_Pos.Y,Robot_Vel.t);	 
...
/*
 * Date: 2022-07-19 13:51:03
 * Functionname: launch_agv::Position_Check()
 * Author: Rolle
 * Email: robot_luoer@163.com
...
void  launch_agv::Publish_Voltage() 
{
	...
	{
		Count_Voltage_Pub = 0;
		voltage_msgs.data = Esp_Origin_Data.data0.powerVoltage_Batter;
		voltage_publisher.publish(voltage_msgs);
	}
}
/*
 * Date: 2022-07-19 13:50:31
...
void launch_agv::Cmd_Vel_Callback(const geometry_msgs::Twist  &twist_aux)
{
	 ...
	 float  datal,datar,vl,vr;  //左轮和右轮
	 //运动学逆解算。vc 和 角速度 解算两个车轮速度
	 vl =(twist_aux.linear.x - (WHEELWIDTH /2) * twist_aux.angular.z ); //运动解算得到左轮速度
	 vr =(twist_aux.linear.x + (WHEELWIDTH /2) * twist_aux.angular.z ); //运动解算得到右轮速度
	//线速度转换为圈数下发
	datal = vl / (PI * WHEELLENGTH); //转换成圈速
	datar = vr / (PI * WHEELLENGTH); //转换成圈速
	//限制圈速
	datar = -(limit(-5.0,datar,5.00)); //对圈速限制幅度
	datal = limit(-5.0,datal,5.00);  // 对圈速限制幅度
	//发布字符串
	string  data_send ="SpeedLR:"+std::to_string(datal)+","+std::to_string(datar)+",\n"; //下发底盘
	const  char *hg_por =data_send.c_str(); //设置发布数据
	 try
	 {
	  Esp32_Serial.write(hg_por); //Sends data to the downloader via serial port //通过串口向下位机发送数据 
	 }
	...
	if((Receive_Data_Serial[0]==FRAME_TAIL_B)&&(Receive_Data_Esp.rxesp[count-2]==FRAME_TAIL_A))//判断是否是尾帧
	{
		...
		{	
			...
			if((CRCA==Receive_Data_Esp.rxesp[count-3])&& (CRCB==Receive_Data_Esp.rxesp[count-4])) //crc正确 存入数据
			{
				...
				{
					databuffer1[i]=Receive_Data_Esp.rxdata1[i];
				}	
				Robot_Vel.Y = 0;				//
				Robot_Vel.vl = Esp_Origin_Data.data1.motorSpeeds[0] * PI * WHEELLENGTH ;//左轮速度提取， 圈速 转换成线速度
				Robot_Vel.vr = -Esp_Origin_Data.data1.motorSpeeds[1] * PI * WHEELLENGTH ;//右轮速度提取，圈速转换成线速度
				Robot_Vel.vc = (Robot_Vel.vr + Robot_Vel.vl)/ 2;  //速度解算 得到中心 坐标线速度
				//Robot_Vel.vc = Esp_Origin_Data.data10.speed;
				//Robot_Vel.w =  Esp_Origin_Data.data10.radianSpeed;
 ...
 * 功能 ：析构函数，实体化对象调用一次。
*/
launch_agv::launch_agv():Sampling_Time(0),Power_voltage(0)
{
	//init the data of all
	memset(&Robot_Pos,0,sizeof(Robot_Pos));
	memset(&Robot_Vel,0,sizeof(Robot_Vel));
	memset(&Send_Data_Esp,0,sizeof(Send_Data_Esp));
	...
	// publisher  size
	voltage_publisher = n.advertise<std_msgs::Float32>("PowerVoltage",10); //发布话题 电压
	...
	if(Uwb_Serial.isOpen())
	{
	 ...
	 try
	 {
	  Esp32_Serial.write(Init_odomc); //Sends data to the downloader via serial port //通过串口向下位机发送数据 
	 }
	 catch (serial::IOException& e)   
	 {
	   ROS_ERROR_STREAM("Unable to send data through serial port"); //If sending data fails, an error message is printed //如果发送数据失败，打印错误信息
...
launch_agv::~launch_agv()
{
	...
	try
	{
	   Esp32_Serial.write(fuck); //Sends data to the downloader via serial port //通过串口向下位机发送数据 
	   Esp32_Serial.write(fuck); //Sends data to the downloader via serial port //通过串口向下位机发送数据 
	}
	catch(serial::IOException& e)   
	{
